...
| **可搜索性** | 每个子问题应能通过当前可用搜索工具、知识库、本地资料或公开来源形成可核验证据；不要依赖某个固定搜索引擎 | 阶段1 大纲 agent |
| **来源补强** | 所有可用搜索工具结果不足时触发补强，优先官方、学术、行业机构、主流媒体和用户资料 | 阶段2 |
...
| 规则 | 说明 | 在哪拦截 |
|:----|:-----|:---------|
| **网页读取可选增强** | Scrapling、webfetch、Jina 或其它网页读取工具可用时优先阅读全文；不可用时基于搜索摘要/知识库/本地资料继续，并降低置信度 | 阶段2 |
...
| **来源多样性硬线** | 补强触发条件：总独立来源 < 8，或来源集中在 ≤3 个域名（数量够但多样性不足）；离线/本地资料模式可放宽但要说明限制 | 阶段2 |
...
| **章节强制并行** | 所有章节必须在同一批次内并行派发（`run_in_background=true`），禁止串行逐一写 | 阶段4 主 agent 循环 |
...
| **清理** | TMPDIR 必须位于授权根目录的 `/tmp/{agent}/{date}/` 虚拟目录；调研结束后只清理本次 TMPDIR，不能使用宿主系统临时目录或跨目录删除 | 阶段5 Step 4 |
...
| **把摘要当全文** | 只能使用搜索摘要时必须降低置信度并标注限制，不得伪装成已阅读全文 | 阶段2 |
