...
这是脚本型浏览器自动化 skill。要真正启动 CDP Proxy 或执行脚本，当前 Agent 必须同时满足：

- 已绑定 `web-access` skill；
- 已绑定 `exec_command` 工具；
- Agent 权限里开启 `execCommand`；
- 系统命令白名单允许 `node`；
- 用户使用的 Chrome/Edge 已开启远程调试授权。

如果 `exec_command` 不可用，不要声称已经打开网页、读取登录态页面或启动 CDP Proxy；只能说明需要开启命令执行权限，或改用当前 Agent 已有的普通搜索/读取工具。

在 Reviva 中运行脚本时优先使用虚拟路径 `/skills/web-access/scripts/...`，不要假定存在 `${CLAUDE_SKILL_DIR}`。下面原始示例里的 `${CLAUDE_SKILL_DIR}` 应替换为 `/skills/web-access`，并优先使用结构化 `exec_command`，例如：

```json
{
  "cmd": "node",
  "args": ["/skills/web-access/scripts/check-deps.mjs"]
}
```

Node.js 22+ 最稳，因为脚本优先使用原生 `WebSocket`。脚本内部对 Node < 22 有 `ws` 模块回退，但普通用户环境不一定能解析到 `ws`；如果检查失败，不要继续网页 AI 字幕或登录态抓取路线，应提示用户升级 Node、安装可用 `ws` 环境，或改用公开接口/普通网页搜索。

对于 Bili Note 等依赖网页 AI 字幕的场景，`web-access` 只是增强能力：只有 `check-deps.mjs` 通过并且浏览器已登录 B 站、目标 tab 可访问时，才可走网页 AI 字幕；否则应回退公开字幕、图文正文、评论或 ASR 路线。
