...
依赖 sympy。用环境检测确认可用的 Python 运行：
    python scripts/generate.py list
    python scripts/generate.py <反应key> <输出.html>
    python scripts/generate.py random <输出.html> [seed]
    python scripts/generate.py spec <reaction-spec.json> <输出.html>
    python scripts/generate.py all <输出目录>

所有生成分支都要求显式输出路径，且拒绝写入 Skill 目录。
"""
...
def _validate_html_output(out_path: Path) -> Path:
    resolved = out_path.expanduser().resolve()
    if resolved.suffix.lower() not in {".html", ".htm"}:
        raise ValueError("输出文件必须使用 .html 或 .htm 扩展名")
    if _is_within(resolved, SKILL_DIR):
        raise ValueError("输出文件不能写入 Skill 目录；请使用当前 Agent 的 outputs 目录")
    return resolved


def _validate_output_directory(out_dir: Path) -> Path:
    resolved = out_dir.expanduser().resolve()
    if _is_within(resolved, SKILL_DIR):
        raise ValueError("输出目录不能位于 Skill 目录内；请使用当前 Agent 的 outputs 目录")
    return resolved


def render_html(data: dict, out_path: Path) -> Path:
    out_path = _validate_html_output(out_path)
    template = TEMPLATE.read_text(encoding="utf-8")
    if PLACEHOLDER not in template:
        raise RuntimeError(f"模板中未找到占位符 {PLACEHOLDER}")
    # Avoid closing the data-island script when a title or explanation contains user text.
...
def _usage() -> str:
    return (
        "用法:\n"
        "  generate.py list\n"
        "  generate.py <reaction-key> <output.html>\n"
        "  generate.py random <output.html> [seed]\n"
        "  generate.py spec <reaction-spec.json> <output.html>\n"
        "  generate.py all <output-directory>"
    )
...
def main(argv):
    ...
    if argv[0] == "list":
        if len(argv) != 1:
            return _usage_error("list 不接受额外参数")
        print("已注册反应:")
        for k in REGISTRY:
            print("  -", k)
        return 0
    ...
    if argv[0] == "random":
        if len(argv) not in {2, 3}:
            return _usage_error("random 需要输出文件，可选第三个参数为 seed")
        import random
        out = Path(argv[1])
        seed = int(argv[2]) if len(argv) == 3 else 0
        key = random.Random(seed).choice(list(REGISTRY))
        written = render_html(K.assemble_data(REGISTRY[key]()), out)
        print(f"written: {written}  (seed={seed} -> {key})")
        return 0

    if argv[0] == "spec":
        if len(argv) != 3:
            return _usage_error("spec 需要 reaction-spec.json 和输出文件")
        spec_path = Path(argv[1]).expanduser().resolve()
        spec = json.loads(spec_path.read_text(encoding="utf-8"))
        if not isinstance(spec, dict):
            raise ValueError("reaction spec 顶层必须是 JSON 对象")
        written = render_html(K.assemble_data(spec), Path(argv[2]))
        print("written:", written)
        return 0

    if argv[0] == "all":
        if len(argv) != 2:
            return _usage_error("all 需要输出目录")
        out_dir = _validate_output_directory(Path(argv[1]))
        out_dir.mkdir(parents=True, exist_ok=True)
        for k in REGISTRY:
            written = render_html(K.assemble_data(REGISTRY[k]()), out_dir / f"{k}.html")
            print("written:", written)
        return 0

    key = argv[0]
    if key not in REGISTRY:
        return _usage_error(f"未知反应 {key}；可用: {', '.join(REGISTRY)}")
    if len(argv) != 2:
        return _usage_error(f"{key} 需要显式输出文件")
    written = render_html(K.assemble_data(REGISTRY[key]()), Path(argv[1]))
    print("written:", written)
    return 0
