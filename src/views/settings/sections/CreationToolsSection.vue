  ...
  <div class="max-w-[880px] mx-auto px-6 py-6">
    <div class="flex items-start justify-between gap-4 mb-5">
      <div>
        ...
        <p class="mt-1 text-[12px] leading-relaxed" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">管理工作台右侧“创作工具”的显示和排序。此处不会影响 Agent 的工具权限。</p>
      </div>
      <button @click="resetDefaults" :disabled="saving" class="h-8 px-3 rounded-lg text-[11px] font-medium border transition-colors disabled:opacity-50 shrink-0" :class="isDark ? 'border-d4 text-wt-aux hover:text-wt-sub hover:bg-white/5' : 'border-bdrF text-lt-aux hover:text-lt-sub hover:bg-l4'">
        <i class="ri-refresh-line" /> 恢复默认
      </button>
    </div>

    <div class="rounded-xl p-3 mb-4 flex items-center gap-3" :class="isDark ? 'bg-d2 border border-d4' : 'bg-l2 border border-bdrF'">
