...
const hasWikiSource = computed(() =>
  (props.wikiItems || []).some((item) => item?.id || item?.wikiId)
);
const canSubmit = computed(
  () => topic.value.trim().length > 0 || usableCtxItems.value.length > 0 || hasWikiSource.value
);
const hint = computed(() => {
    ...
    <template #header>
      <div class="flex items-center gap-2.5">
        <div class="w-7 h-7 rounded-lg flex items-center justify-center bg-amber-400/14">
          <i class="ri-node-tree text-[14px] text-amber-500" />
        </div>
        <div>
          <div class="text-[13px] font-bold" :class="isDark ? 'text-wt-main' : 'text-lt-main'">
            生成知识图谱
          </div>
    ...
    <div class="space-y-3">
          ...
          <button
            ...
            :class="
              mode === 'local'
                ...
                : isDark
                  ? 'border border-d4 hover:border-amber-400/20 bg-d0'
                  : 'border border-bdrF hover:border-amber-200 bg-white'
            "
          >
            <div class="flex items-center gap-1.5 mb-0.5">
              <i
                class="ri-computer-line text-[11px]"
                :class="
                  mode === 'local'
                    ? isDark
                      ? 'text-amber-400'
              ...
              >
              <span
                class="ctx-pill text-[8px] ml-auto"
                :class="
                  isDark
            ...
            <p class="text-[9px] leading-tight" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">
              本机模型单次抽取 JSON
            </p>
          </button>
          <button
            ...
            :class="
              mode === 'cloud'
                ...
                : isDark
                  ? 'border border-d4 hover:border-sky-400/20 bg-d0'
                  : 'border border-bdrF hover:border-sky-200 bg-white'
            "
          >
            <div class="flex items-center gap-1.5 mb-0.5">
              <i
                class="ri-cloud-line text-[11px]"
                :class="
                  mode === 'cloud'
                    ? isDark
                      ? 'text-sky-400'
      ...
      >
        <div class="flex items-center gap-2 mb-2">
          ...
          >
          <div class="flex-1" />
          <span class="text-[9px]" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'"
            >{{ topic.length }} 字</span
          >
        </div>
        <textarea
