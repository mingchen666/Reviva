  ...
  <div class="h-full flex flex-col" :class="isDark ? 'bg-d1' : 'bg-l1'">
    <div class="h-12 px-3 flex items-center justify-between border-b shrink-0" :class="isDark ? 'border-d4' : 'border-bdrL'">
      <div class="min-w-0">
        <div class="flex items-center gap-2">
          <span class="text-[14px] font-semibold" :class="isDark ? 'text-wt-main' : 'text-lt-main'">页面</span>
        ...
        <p class="mt-0.5 text-[11px]" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">可阅读知识页</p>
      </div>
      <button
    ...
    <div class="flex-1 overflow-y-auto px-2 py-3">
      <template v-if="hasWiki">
        <div v-if="rootPages.length" class="mb-4">
          <div class="px-2 pb-1.5 flex items-center gap-1.5">
            ...
            <span class="text-[10px]" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">{{ rootPages.length }}</span>
          </div>
          <button
            ...
            <span class="text-[13px] truncate">{{ page.path }}</span>
          </button>
        </div>
        ...
        <div>
          <div class="px-2 pb-1.5 flex items-center gap-1.5">
            ...
            <span v-if="contentPages.length" class="text-[10px]" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">{{ contentPages.length }}</span>
          </div>
          <button
            ...
            <span class="text-[13px] truncate">{{ page.path.replace(/^pages\//, '') }}</span>
          </button>
      ...
      <div v-else class="h-full flex flex-col items-center justify-center text-center px-4 gap-2">
        ...
        <p class="text-[13px]" :class="isDark ? 'text-wt-dim' : 'text-lt-aux'">选择或创建一个 Wiki</p>
      </div>
    </div>
  </div>
</template>
